...
class SwapyInstance {
  ...
  private _createSlotElMap() {
    return (Array.from(this._rootEl.querySelectorAll("[data-swapy-slot]")) as HTMLElement[]).reduce(
      (map, el) => {
  ...
  private _createItemElMap() {
    return (Array.from(this._rootEl.querySelectorAll("[data-swapy-item]")) as HTMLElement[]).reduce(
      (map, el) => {
